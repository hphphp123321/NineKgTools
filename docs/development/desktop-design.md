...
- **文件过滤 → 高级过滤规则**：`ignored_files` / `ignored_patterns` / `allowed_extensions` 三组列表**已在客户端可编辑**（不再要求改 config.yaml），收在「高级过滤规则」`Expander`（默认收起）里。每组用可复用 chip 编辑器 **`Views/Components/StringListEditor`**（DataContext = `StringListEditorViewModel`）：现有项是带 ✕ 的 chip，末尾输入框回车 / ﹢ 添加，另有"恢复默认"。`SettingsViewModel` 持 3 个实例，订阅各自 `Items.CollectionChanged` 写回 `config.Files.*` + `DebouncedSave`；扫描时实时读取无需重启。扩展名经 `NormalizeExtension`（小写 + 补 `.`）规范化。**改 `FilesConfig` 字段默认值时，同步 `SettingsViewModel` 里三个 editor 的 `defaults`**
