...
3. **Z2**：`#99000000` 暗化层（60% 黑）—— 保证任何封面色下文字 ≥ 4.5:1 contrast
4. **Z3**：`RadialGradientBrush` 径向 vignette（中心透明 → 边缘 #80 黑），聚焦中央内容
5. **Z3b**：顶部 200px `LinearGradientBrush` 高光暗化（`#40 → #00`），让 28px Hero 标题在任何封面色上都清晰

**预渲染管线**（性能关键）：
- 不用 Avalonia 实时 `BlurEffect`（每帧 GPU 重算大窗口可能掉帧）
- 用 ImageSharp `GaussianBlur(60)` 一次性离线算 + 缩到 400×600 + Jpeg 85 质量编码为 byte[]
...
- **暗化层固定 60%**（不按主题动态调）：实测多数封面是暗色调，60% 黑在亮 / 暗主题下都让文字保持可读
- **降采样到 400×600**：60px 模糊后细节无法分辨，省 75% 内存
