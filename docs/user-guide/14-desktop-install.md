...
1. 下载 `NineKgTools-Desktop-win-x64-portable.zip`，解压到任意目录。
2. 双击 `NineKgTools.Desktop.exe`。
3. 解压目录里自带一个 `.portable` 空标记文件——**有它则数据落在 exe 同目录的 `data/`**（而非 `%LocalAppData%`），整个目录拷到别的机器即可带着数据走。
...
1. 按你的芯片下载对应 `desktop-v*` 包：Apple Silicon 选 `osx-arm64`，Intel 选 `osx-x64`。
2. 拖到「应用程序」。
3. **首次打开被 Gatekeeper 拦截**（应用未签名/未公证）：按住 Control 右键点图标 →「打开」→ 确认。一次性。

> macOS 为 best-effort 支持：x64 自动更新可用，**Apple Silicon 的自动更新本期未接通**（手动下载新版即可）。

---

## Linux（best-effort）

1. 下载 `desktop-v*` 的 Linux 包（AppImage）。
2. `chmod +x` 后直接运行。
