...
> **本文只讲 Web / Docker 服务端。** 如果你要的是本机原生的**桌面端**（Avalonia，Win/Mac/Linux，带托盘 / 右键识别 / 自动更新），那是另一套东西、数据独立、不走 Docker——安装见 [用户指南 → 14 桌面端安装](../user-guide/14-desktop-install.md)。下表的「Windows 便携版」指的是 **Web 应用**的便携 zip（浏览器访问），不是桌面端。
